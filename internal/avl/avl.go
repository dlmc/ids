...
// Copied from github.com/emirpasic/gods/trees/avltree
// Enhanced for imds use
...
func (t *Tree) Put(key interface{}, value interface{}) bool {
	var created bool
	t.put(key, value, nil, &t.Root, &created)
	return created
}
...
func (t *Tree) put(key interface{}, value interface{}, p *Node, qp **Node, created *bool) bool {
	q := *qp
	if q == nil {
		t.size++
	...
	if c == 0 {
		//do not set if the key already exist, uncomment if overwrite the existing key 
		//is required
		//q.Key = key
		//q.Value = value
		*created = false
	...
	a := (c + 1) / 2
	var fix bool
	fix = t.put(key, value, q, &q.Children[a], created)
	if fix {
...
func (t *Tree) remove(key interface{}, qp **Node, found *bool) bool {
	q := *qp
	if q == nil {
		return false
	}
	...
	a := (c + 1) / 2
	fix := t.remove(key, &q.Children[a], found)
	if fix {
