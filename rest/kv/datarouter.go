...
func (s *dataGet) ServeHTTPWithCtx(c ghttp.Ctx, h *ghttp.Http) ghttp.Ctx {
	...
	if err == nil {
		if st, found := store.GetStore(sn); !found {
			...
			err = errors.New(strKeyNotExist + k)
		}
	}
...
// POST /data?storename=sname&k=key&v=value 
// POST /data?storename=sname&k=key and v in the body
type dataPost struct {}
func (d *dataPost) ServeHTTPWithCtx(c ghttp.Ctx, h *ghttp.Http) ghttp.Ctx {
	reqUri := h.R.RequestURI
	log := h.Log.Str("Post", reqUri).Logger()
	sn, k, v, err := decodeRequestNKV(h)

	if err == nil {
		if st, found := store.GetStore(sn); !found {
			err = errors.New(strStoreNotExist + sn)
		} else if ok := st.Create(k, v); !ok {
			err = errors.New(strKeyExist + k)
		}
	}
...
// PUT /data?storename=sname&k=key&v=value
// PUT /data?storename=sname&k=key and v in the body
type dataPut struct {}
...
// Delete a k/v pair
// DELETE /data?storename=sname&k=key
